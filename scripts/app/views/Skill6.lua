...
local Skill6 = class("Skill6", function()
    local layer = display.newNode()
    require("framework.api.EventProtocol").extend(layer)    -- 添加事件
    return layer
end)

function Skill6:ctor( skillModel,atkerView,targetsView,atker,targets )
...
-- 移除自身的方法
function Skill6:onRemoveSelf_( event )
    self:removeSelf()
    -- self.skillModel_:endAtk()
end

-- 进入攻击的状态
function Skill6:beginAtk_( event )
    -- 在攻击的动作中会调用父方法
    display.addSpriteFramesWithFile("frames/frame5.plist", "frames/frame5.png")
    local atker = event.atker
    local targets = event.targets
    for i=1,#targets do
        local target = targets[i]
        local targetview = self.targetsView_[target:getNickName()]
        local skillEffect = display.newSprite("#frame5_0.png"):addTo(self)

        local side = target:getSide() == 0 and -1 or 1
        skillEffect:setScaleX(side)
        skillEffect:setPosition(ccp(targetview:getPositionX() - side * 40,targetview:getPositionY() + 40))


        local frames1 = display.newFrames("frame5_%d.png", 0, 15)
        local animation1 = display.newAnimation(frames1, 1 / 24)
        local frames2 = display.newFrames("frame5_%d.png", 15, 7)
        ...
        actions[#actions + 1] = CCCallFunc:create(function (  )
            display.removeSpriteFramesWithFile("frames/frame5.plist", "frames/frame5.png")
        end)
...
function Skill6:cancelSkillAtk_( event )
    self:removeSelf()
end

function Skill6:onEndEvent_( event )
    self:removeSelf()
end

-- 进入死亡的状态
function Skill6:enterDie_( event )
